...
class TMDbApi:
  ...
  def query(self, qparams):
      ...
      monoling_response = list(self.ml_index.mquery(qparams.source_lang, qparams.limit, [q_o_tags for q, q_o_tags in q_out_tags], filter = [f for f in dic_filter]))

      loopArray = zip(qparams.qlist, qparams.qinfo, monoling_response)
  ...
  def _query(self, q, qinfo, ml_response, qparams):
    ...
    map_docs = None
    try:
      map_docs = self._msrc_id2tgt_id(src_hits, qparams.source_lang, qparams.target_lang, return_multiple=True, source_metadata=qparams.source_metadata, target_metadata=qparams.target_metadata, domains=qparams.domains)
    except ValueError:
  ...
  def _match(self, qstring, qinfo, l_best_segments, qparams):
    ...
    for segment, ter in segments: # This one is for each segment
      ...
      # if timer() - self.timer.ts.get("match_time_query", None) > wait_time:
  ...
  def _msrc_id2tgt_id(self, src_hits, source_lang, target_lang, return_multiple=False, source_metadata=None, target_metadata=None, domains=None):
    margs = [(uuid.UUID(src_hit.meta.id), source_lang, target_lang, source_metadata, target_metadata, domains) for src_hit in src_hits]
